    ...
    public class APILogListener : PokemonGo.RocketAPI.ILogger
    {
        ...
        public void HashStatusUpdate(HashInfo info)
        {
            ...
            if (session.Settings.DisplayVerboseLog && lastVerboseLog< DateTime.Now.AddSeconds(-60))
            {
                ...
                Logger.Write($"(HASH SERVER) Key[{info.MaskedAPIKey}] Last 1 minute  {info.Last60MinAPICalles} request/min , AVG: {info.Last60MinAPIAvgTime:0.00} ms/request , Fastest : {info.Fastest}, Slowest: {info.Slowest}", LogLevel.Info, ConsoleColor.White);
            }
            session.EventDispatcher.Send(new StatusBarEvent($"[{info.MaskedAPIKey}] - {info.Last60MinAPICalles} request/min , AVG: {info.Last60MinAPIAvgTime:0.00} ms/request , Fastest : {info.Fastest}, Slowest: {info.Slowest}"));
        }
