    ...
    public class SelectBuddyPokemonTask
    {
        public static async Task Execute(ISession session, CancellationToken cancellationToken, ulong pokemonId = 0)
        {
            ...
            if (pokemonId == 0)
            {
                ...
                {
                    var currentBuddy = (await session.Inventory.GetPokemons()).FirstOrDefault(x => x.Id == session.Profile.PlayerData.BuddyPokemon.Id);
                ...
                var buddy = (await session.Inventory.GetPokemons()).Where(x => x.PokemonId == buddyPokemonId)
            ...
            if (newBuddy == null)
            {
                newBuddy = (await session.Inventory.GetPokemons()).FirstOrDefault(x => x.Id == pokemonId);
            }
